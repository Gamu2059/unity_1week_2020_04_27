...
public class LowResolutionCamera : MonoBehaviour
{
    ...
    /// デフォルト解像度
    /// </summary>
    private readonly Vector2 RESOLUTION = new Vector2(192, 108);
    ...
    public void SetResolution(float resolutionWeight)
    {
        ...
        if (_subCamera == null)
        {
            ...
            _subCamera.cullingMask = _camera.cullingMask;
            _subCamera.clearFlags = _camera.clearFlags;
            _subCamera.fieldOfView = _camera.fieldOfView;
            _subCamera.depth = _camera.depth;
            _subCamera.transform.parent = transform;
            _subCamera.transform.localRotation = Quaternion.identity;
        }
